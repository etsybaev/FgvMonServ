...
<body style="background-color: #f0f4ce">
    <div>
        ...
        <h1>Add/Edit Status of Call</h1>

        <c:url var="addStatusOfCallAction" value="/admin/statusofcallmanagement/addstatusofcall"/>
        <form:form action="${addStatusOfCallAction}" commandName="statusOfCall">
            <table>
                <c:if test="${!empty statusOfCall.id}">
                    <tr>
                        <td>
                            <form:label path="id">
                ...
                <tr>
                    <td colspan="2">
                        <c:if test="${!empty statusOfCall.id}">
                            ...
                            <input type="button" onclick="location.href='/admin/statusofcallmanagement';"
                                   value="<spring:message text="Discard changes"/>"/>
                        </c:if>
                        <c:if test="${empty statusOfCall.id}">
                            <input type="submit" value="<spring:message text="Add Status Of Call"/>"/>
                        </c:if>
                    </td>
                </tr>
            </table>
        ...
        <c:if test="${!empty statusOfCallList}">
            <table class="sortable" id="sortableTable">
                ...
                <c:forEach items="${statusOfCallList}" var="statusOfCall">
                    <tr>
                        <td>${statusOfCall.id}</td>
                        <td>${statusOfCall.status}</td>
                        <td><a href="<c:url value='/admin/statusofcallmanagement/editstatusofcall/${statusOfCall.id}'/>">Edit</a></td>
                        <td><a href="javascript:deleteScript.deleteStatusOfCallRecord(${statusOfCall.id})">Delete</a></td>
                    </tr>
                </c:forEach>
            </table>
        </c:if>
